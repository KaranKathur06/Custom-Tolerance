-- Use the server-verified admin identity for moderation RPC authorization.
-- The existing RPC depends on auth.uid(), which is not reliable when invoked
-- from a server route using service-role database access.

...
`POST /api/products/images` requires `productId`, `expectedVersion`, and the image file; it returns `{ success: true, media, productVersion }`. A stale version returns `409 CONFLICT_STALE_DRAFT` without storing an object or metadata, so the client reloads/reconciles before retrying. Delete and reorder requests likewise require `expectedVersion` and return the incremented version. It returns safe typed errors such as `PRODUCT_MEDIA_INVALID_TYPE`, `PRODUCT_MEDIA_TOO_LARGE`, `PRODUCT_MEDIA_LIMIT_REACHED`, `PRODUCT_MEDIA_BUCKET_MISSING`, `PRODUCT_MEDIA_STORAGE_UNAVAILABLE`, and `PRODUCT_ACCESS_DENIED`; provider messages are logged server-side only.

`POST /api/dashboard/seller/products/{id}/publish` requires `expectedVersion` and returns `{ success: true, product, approval, productVersion }`. It returns `409 CONFLICT_STALE_DRAFT` for a stale version and `409 PRODUCT_ALREADY_PENDING` only when the request cannot be idempotently resolved to the existing pending approval. Validation failures are `422` with field errors; ownership and lifecycle errors use `401`, `404`, and `409` as appropriate.

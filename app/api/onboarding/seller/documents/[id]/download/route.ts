...
export async function GET(request: Request, { params }: { params: { id: string } }) {
  ...
  const signed = await createDocumentSignedUrl(auth.supabase, auth.user.id, request, result.document, "DOCUMENT_DOWNLOADED");
  ...
  return NextResponse.json({ success: true, url: signed.url, expiresAt: new Date(Date.now() + 300000).toISOString(), fileName: result.document.original_filename, mimeType: result.document.mime_type });
}

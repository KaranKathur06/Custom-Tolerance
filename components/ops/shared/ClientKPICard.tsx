...
export function ClientKPICard({ title, value, change, changeLabel, icon: Icon, variant = 'default', sparkline }: ClientKPICardProps) {
  ...
  return (
    <div className={`ops-kpi-card ${variant}`}>
      <div className="ops-kpi-header">
        ...
        {Icon && <div className="ops-kpi-icon-wrap"><Icon className="w-4 h-4" /></div>}
      </div>
      ...
      {sparkline && sparkline.length > 0 && (
        <div className="ops-kpi-sparkline">
          <svg viewBox={`0 0 ${sparkline.length * 12} 24`} className="ops-sparkline-svg">
            <polyline fill="none" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" points={sparkline.map((v, i) => `${i * 12},${24 - (v / Math.max(...sparkline)) * 20}`).join(' ')} />
          </svg>
        </div>
      )}
    </div>
